...
if(isset($_GET['draw']))
{
    ...
    {
        $teacher = $db->single('teachers',['user_id'=>$user->id]);
        $where .= " AND teacher_id=$teacher->id";
    }
    
    $db->query = "SELECT *, IF(type=1,'Penilai','Teman Sejawat') type FROM $table $where ORDER BY ".$col_order." ".$order[0]['dir']." LIMIT $start,$length";
    $data  = $db->exec('all');
