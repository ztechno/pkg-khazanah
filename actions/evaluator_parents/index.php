...
if(isset($_GET['draw']))
{
    ...
    else
    {
        ...
        $db->query = "SELECT * FROM $table $where GROUP BY parent_id ORDER BY ".$col_order." ".$order[0]['dir']." LIMIT $start,$length";
    ...
    foreach($data as $key => $d)
    {
        ...
        if(is_allowed(get_route_path('evaluator_parents/edit',[]),auth()->user->id)):
            $action .= '<a href="'.routeTo('evaluator_parents/edit',['id'=>$d->id]).'" class="btn btn-sm btn-warning"><i class="fas fa-pencil-alt"></i> Edit</a>';
        endif;
        if(is_allowed(get_route_path('crud/delete',['table'=>$table]),auth()->user->id)):
            $action .= '<a href="'.routeTo('evaluator_parents/delete',['table'=>$table,'id'=>$d->id]).'" onclick="if(confirm(\'apakah anda yakin akan menghapus data ini ?\')){return true}else{return false}" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i> Hapus</a>';
        endif;
        $results[$key][] = $action;
    }
